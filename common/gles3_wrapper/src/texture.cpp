...
TextureId stbCreateTexture(const std::string_view filename, Texture::TextureFlags flags)
{
	...
	EASY_END_BLOCK;
#endif
	Image image = StbImageConvert(textureFile, true);
	/*if (extension == ".hdr")
	{
...
void Texture::CreateTexture()
{
	...
	glTexParameteri(GL_TEXTURE_2D,
	                GL_TEXTURE_WRAP_S,
	                flags_ & CLAMP_WRAP ? GL_CLAMP_TO_EDGE : GL_REPEAT);
	glTexParameteri(GL_TEXTURE_2D,
	                GL_TEXTURE_WRAP_T,
	                flags_ & CLAMP_WRAP ? GL_CLAMP_TO_EDGE : GL_REPEAT);
	glTexParameteri(GL_TEXTURE_2D,
	                GL_TEXTURE_MAG_FILTER,
	                flags_ & SMOOTH_TEXTURE ? GL_LINEAR : GL_NEAREST);
	glCheckError();
	if (flags_ & MIPMAPS_TEXTURE)
	{
		glTexParameteri(GL_TEXTURE_2D,
		                GL_TEXTURE_MIN_FILTER,
		                flags_ & SMOOTH_TEXTURE
